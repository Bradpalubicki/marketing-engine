// HVAC Campaign Factory
// Triggered when vertical_tag = 'hvac' AND completeness_score >= 60
// All campaigns created PAUSED, MANUAL_CPC, $2.00 ceiling
// Spec: HVAC Vertical Bible v1 + Phase 3 spec

import { inngest } from '../client'
import { supabaseAdmin } from '@/lib/supabase'
import { campaignExistsByName, createCampaignWithDryRun } from '@/lib/google-ads'

interface HvacFactoryEvent {
...
// HVAC campaign structure per Bible Part 2.1 and spec Track 3 section 3.7
const HVAC_CAMPAIGNS = [
  { name: 'Emergency Repair', budgetPct: 0.55, keywordCluster: 'emergency' },
  { name: 'Replacement', budgetPct: 0.30, keywordCluster: 'replacement' },
  { name: 'Maintenance', budgetPct: 0.15, keywordCluster: 'maintenance' },
] as const

const CPC_BID_CEILING_MICROS = 2_000_000 // $2.00

export const hvacCampaignFactory = inngest.createFunction(
  { id: 'hvac-campaign-factory', name: 'HVAC Campaign Factory' },
  { event: 'campaign/hvac.requested' },
  async ({ event, step }) => {
    const { organizationId, locationId, customerId, monthlyBudget, locationName, city } =
      event.data as HvacFactoryEvent
    ...
    if (!intel || intel.completeness_score < 60 || intel.vertical_tag !== 'hvac') {
    ...
    for (const campaign of HVAC_CAMPAIGNS) {
      const campaignName = `HVAC — ${campaign.name} — ${city}`
      const dailyBudgetMicros = Math.round((googleBudget * campaign.budgetPct / 30.44) * 1_000_000)

      const result = await step.run(`create-hvac-${campaign.keywordCluster}`, async () => {
        ...
        // Create PAUSED campaign with validateOnly check
